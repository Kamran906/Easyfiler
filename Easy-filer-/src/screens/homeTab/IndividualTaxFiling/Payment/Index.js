...
const Payment = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (res.data && res.data.incorporation_type && res.data.incorporation_type.package) {
        const newData = res.data.incorporation_type.package;
        newData.id = Date.now(); // Ensure unique ID

        const existingData = await AsyncStorage.getItem('responseArray');
        let dataArray = existingData ? JSON.parse(existingData) : [];

        // Check if the new data already exists in the dataArray by a unique identifier
        const itemExists = dataArray.some(item => item.id === newData.id);
    ...
    <View style={Style.container}>
      ...
      <View style={{
        ...
        <View style={{ flexDirection: "row", justifyContent: "flex-end", width: responsiveScreenWidth(90) }}>
          ...
          <TouchableOpacity>
            <Text style={{ color: "red", fontSize: responsiveScreenFontSize(1.8), fontFamily: FontFamily.MONTSERRAT_Regular }}>Click here</Text>
          </TouchableOpacity>
        </View>
