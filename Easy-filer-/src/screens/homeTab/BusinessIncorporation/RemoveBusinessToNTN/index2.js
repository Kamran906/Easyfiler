...
const RemoveFormUpload = () => {
  ...
  const [imagesList, setImageList] = useState([
    {
      id: '0',
      name: 'Color copy of CNIC',
      img: null,
    },
    {
      id: '1',
      name: 'Rental agreement or ownership Document',
      img: null,
    },
    {
      id: '2',
      name: 'Utility bill last paid',
      img: null,
    },
    {
      id: '3',
  ...
  const irisPassword = useSelector((state) => state.salary.irisPassword);
  ...
  const Submit = async () => {
    ...
    formData.append('business[iris_password]', irisPassword);
    formData.append('business[incorporationtype_id]', incorporationtypeId);

    const imageFields = [
      'color_copy_cnic',
      'rent_agreement',
      'utility_bill_last_paid',
      'company_letter_head',
    ];
  ...
  const UpdatePost = async (formData) => {
    ...
    try {
      axios({
        url: BASE_URL + `users/${userId}/business-incorporations/${registrationRemove}`,
        method: 'Post',
        data: formDataToSend,
        headers: {
          'Content-Type': 'multipart/form-data',
