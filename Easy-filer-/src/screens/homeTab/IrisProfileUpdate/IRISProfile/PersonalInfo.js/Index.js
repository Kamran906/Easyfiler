import { View, Text, Pressable, TouchableOpacity, Modal, ScrollView, Image } from 'react-native'
...
const PersonalInfo = () => {
  ...
  const fetcData = async (id) => {
    ...
    try {
      ...
      if (Object.keys(IrisData).length > 0) {
        ...
        setCompanyName(lastIrisData.employer)
        ...
        if (lastIrisData.irisbankaccunts && lastIrisData.irisbankaccunts.length > 0) {
          console.log('lastIrisData.irisbankaccunts', lastIrisData.irisbankaccunts);
          const bankDetailsArray = lastIrisData.irisbankaccunts.map(bankAccount => ({
            bankName: bankAccount.bankName,
            iban: bankAccount.iban,
          }));
          console.log('bankDetailsArray', bankDetailsArray); // Log the mapped array for inspection
  ...
  const renderedBankDetails = bankDetails.map((item, index) => (
    ...
    <View key={index} style={{ flexDirection: "row", justifyContent: "space-between" }}>
      <View>
        <Text style={{ color: "black" }}>Bank Name: {item.bankName}</Text>
            ...
            <View style={{ justifyContent: "center", alignItems: "center" }}>
              ...
              <View style={{ marginTop: responsiveScreenHeight(2) }}>
                <TextInput
                  ...
                  style={[Style.placeholder1, { color: companyError ? 'red' : 'black' }]}
                  outlineStyle={[Style.placeholder2, { borderColor: companyError ? 'red' : 'black' }]}
                  outlineColor='black'
                  activeOutlineColor={companyError ? 'red' : 'black'}
